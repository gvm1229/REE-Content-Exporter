...
After setup, wizard v0.2 asks whether to export one mesh or import a CSV for batch mesh export. If CSV batch mode is selected, the wizard then asks how skeletal meshes should be handled: either prompt for animations when each skeletal mesh is found, or auto-export skeletal meshes without prompting for animations. CSV batch files must contain exactly one column of primary mesh names or paths. A header row is optional when the first cell is `mesh`, `mesh_name`, or `name`; otherwise every non-empty row is treated as a mesh query. Path prompts accept pasted Windows-style paths with surrounding double quotes, such as `"./all_meshes.csv"`. The wizard rejects blank rows, duplicate mesh entries, and files with extra columns before generating a script.
