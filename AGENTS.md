...
- Treat `Noise3D_00_MSK1` texture export errors as warnings only, including versioned `.tex.*` paths; this known 3D noise texture is unsupported by the TEX reader. Do not broaden this exception: every other texture source, read, DDS, or PNG conversion error must remain fatal.
